...
# - This is the script that handles the init process an uses the supervisor system 
# - to start the daemons to run and also catch signals (SIGTERM) to shutdown all processes 
# - started by supervisord. This is a modified version of an init script made by Phusion.
#
...
def import_envvars(clear_existing_environment = True, override_existing_environment = True):
	if not os.path.exists("/etc/unit_environment"):
		return
	new_env = {}
	for envfile in listdir("/etc/unit_environment"):
		name = os.path.basename(envfile)
		with open("/etc/unit_environment/" + envfile, "r") as f:
...
def export_envvars(to_dir = True):
	...
	for name, value in os.environ.items():
		if name in ['HOME', 'USER', 'GROUP', 'UID', 'GID', 'SHELL']:
			continue
		if to_dir:
			with open("/etc/unit_environment/" + name, "w") as f:
				f.write(value)
		shell_dump += "export " + sanitize_shenvname(name) + "=" + shquote(value) + "\n"
	with open("/etc/unit_environment.sh", "w") as f:
		f.write(shell_dump)
	with open("/etc/unit_environment.json", "w") as f:
...
def run_startup_files():
	# script execute once when starting in /etc/unit_init.once/*
	for name in listdir("/etc/unit_init.once"):
		filename = "/etc/unit_init.once" + name
		if is_exe(filename):
			info("Running %s..." % filename)
			run_command_killable_and_import_envvars(filename)
			os.remove(filename)

	# script execute when starting in /etc/unit_init.d/*
	for name in listdir("/etc/unit_init.d"):
		filename = "/etc/unit_init.d" + name
		if is_exe(filename):
			info("Running %s..." % filename)
			run_command_killable_and_import_envvars(filename)
...
def shutdown_runit_services(quiet = False):
	...
	os.system("/usr/bin/sv down /etc/unit_services/*")

def wait_for_runit_services():
	debug("Waiting for runit services to exit...")
	done = False
	while not done:
		done = os.system("/usr/bin/sv status /etc/unit_services/* | grep -q '^run:'") != 0
		if not done:
			time.sleep(0.1)
...
parser.add_argument('--skip-startup-files', dest = 'skip_startup_files',
	action = 'store_const', const = True, default = False,
	help = 'Skip running /etc/unit_init.once/*, /etc/unit_init.d/* and /etc/rc.local')
