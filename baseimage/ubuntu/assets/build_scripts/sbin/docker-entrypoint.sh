...
function import_envvars () {
    ...
    if [ -d "${unit_envvar_root}" ] && [ ! -n "$(find ${unit_envvar_root} -prune -empty -type d)" ]; then
...
function run_startup_files() {
    # execute pre-service scripts when starting
    if [ -d "${unit_run_root}" ] && [ ! -n "$(find ${unit_run_root} -prune -empty -type d)" ]; then
...
function start_runit () {
    ...
    /usr/bin/runsvdir -P "${unit_services_root}" 'log:.........................................................................................................' &
    runsvdir_PID=$!
...
function shutdown_runit_services () {
    ...
    /usr/bin/sv down "${unit_services_root}"/*

    # give some time and check if service is down
    count=1
    while [ $(/usr/bin/sv status "${unit_services_root}"/* | grep -c "^run:") != 0 ]; do
        sleep 1
        count=`expr $count + 1`;
        if [ $count -gt 10 ]; then
            break;
