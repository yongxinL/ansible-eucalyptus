...
def run_startup_files():
	# Run /etc/runonce/*
	info("Run files in /etc/runonce/")
	for name in listdir("/etc/runonce"):
		filename = "/etc/runonce/" + name
		if is_exe(filename):
			info("Running %s..." % filename)
			run_command_killable_and_import_envvars(filename)
			os.remove(filename)
        # Run /etc/runalways/*
	info("Run files in /etc/runalways/")
	for name in listdir("/etc/runalways"):
		filename = "/etc/runalways/" + name
		if is_exe(filename):
			info("Running %s..." % filename)
			run_command_killable_and_import_envvars(filename)
...
parser.add_argument('--skip-startup-files', dest = 'skip_startup_files',
	action = 'store_const', const = True, default = False,
	help = 'Skip running /etc/runonce/*, /etc/runalways/*  and /etc/rc.local')
