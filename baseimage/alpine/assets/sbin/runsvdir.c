...
#define USAGE " [-P] dir (Note: this is the hacked version for Docker!"
...
/*
 * signals services to shudown and waits for them to exit until
 * max_shutdown in milliseconds, smallest time is 25 milliseconds.
 * modified for running in docker.
 */
 int shutdown(int max_shutdown) {
  int i;
  int alive;
  pid_t pid;
  ...
  while(alive && max_shutdown--) {
    usleep(25000);

    // check rep children.
    for (i =0; i < svnum; i++) {
      if (sv[i].pid) {
        pid = waitpid(sv[i].pid, NULL, WNOHANG);
        if (pid > 0) {
...
int main(int argc, char **argv) {
  ...
  /*
   * redirect stderr to stdout, modified for running in docker.
   */
